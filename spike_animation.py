...
def spike_animation(
    ...
    for p in spikes:
        spikes_ex[p]["texbin"] = (
            (spikes_ex[p]["t"] - spikes_ex[p]["t"].min()) / 50
        ).astype(np.int)
    counts_ex = (
        spikes_ex["Xe"]
        .groupby(["texbin", "x", "y"])["t"]
        .count()
        .reset_index(level=["x", "y"])
    )
    ...
    img = axX.imshow(np.zeros((imgsize, imgsize)), vmin=0, vmax=max_counts)
    dotsX, = axX.plot([], [], "r.")
