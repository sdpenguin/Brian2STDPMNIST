...
def sparsenMatrix(baseMatrix, pConn):
    ...
    while numWeights < numTargetWeights:
        idx = (np.int32(np.random.rand(
        ) * baseMatrix.shape[0]), np.int32(np.random.rand() * baseMatrix.shape[1]))
...
def create_weights():
    ...
    for name in connNameList:
        ...
        if pConn['ee_input'] < 1.0:
            weightMatrix, weightList = sparsenMatrix(
                weightMatrix, pConn['ee_input'])
        else:
            weightList = [(i, j, weightMatrix[i, j])
                          for j in range(nE) for i in range(nInput)]
    ...
    for name in connNameList:
        if nE == nI:
            ...
            for i in range(nI):
                weightMatrix[i, i] = 0
            weightList = [(i, j, weightMatrix[i, j])
                          for i in range(nI) for j in range(nE)]
        else:
