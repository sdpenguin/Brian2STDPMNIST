...
def plot_2d_input_weights():
    ...
    im2 = imshow(weights, interpolation="nearest",
                 vmin=0, cmap=chosenCmap)  # my_cmap
    colorbar(im2)
...
#         # im = imshow(values, interpolation="nearest", cmap=cm.get_cmap('spectral'))  # copper_r   autumn_r  Greys  my_cmap  gist_rainbow
    # copper_r   autumn_r  Greys  my_cmap  gist_rainbow
    im = imshow(values, interpolation="nearest",
                cmap=chosenCmap, aspect='auto')
    cbar = colorbar(im)
...
for label, x, y in zip(list(range(200)), XA_sum, AA_sum):
    plt.annotate(label,
                 xy=(x, y), xytext=(-0, 0),
