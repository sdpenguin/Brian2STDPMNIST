...
class DiehlAndCookExcitatoryNeuronGroup(DiehlAndCookBaseNeuronGroup):
    ...
    def create_equations(self):
        self.model += b2.Equations(
            """
            dv/dt = ((v_rest_e - v) + (I_synE + I_synI) / nS) / tau_e  : volt (unless refractory)
            I_synI = gi * nS * (v_eqm_synI_e - v)  : amp
...
class DiehlAndCookInhibitoryNeuronGroup(DiehlAndCookBaseNeuronGroup):
    ...
    def create_equations(self):
        self.model += b2.Equations(
            """
            dv/dt = ((v_rest_i - v) + (I_synE + I_synI) / nS) / tau_i  : volt (unless refractory)
            I_synI = gi * nS * (v_eqm_synI_i - v)  : amp
            """
        )
